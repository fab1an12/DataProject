...
from extractLeisureFromOPMS import contar_ocio_en_barrios
...
def crear_tabla_y_insertar_datos(df, db_config):
    try:
        ...
        # Crear la tabla
        create_table_query = """
        CREATE TABLE IF NOT EXISTS zonas_de_ocio (
            id SERIAL PRIMARY KEY,
            barrio VARCHAR(255) NOT NULL,
            numero_ocio INT NOT NULL
        );
        """
        cursor.execute(create_table_query)
        print("Tabla 'zonas_de_ocio' creada o verificada correctamente.")

        # Insertar los datos
        insert_query = """
        INSERT INTO zonas_de_ocio (barrio, numero_ocio)
        VALUES (%s, %s);
        """
        for _, row in df.iterrows():
            cursor.execute(insert_query, (row['Barrio'], row['NumeroOcio']))
